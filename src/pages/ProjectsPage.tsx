  ...
  <section className="mx-auto max-w-6xl px-6 py-24">
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {projects.map((project) => (
          <article
            key={project.id}
            className="group flex flex-col gap-4 rounded-3xl border border-white/70 bg-white/90 p-8 shadow-soft-lg backdrop-blur transition-transform hover:-translate-y-1"
          >
            <div className="text-sm uppercase tracking-[0.3em] text-apple-gray/50">
              {project.year}
            </div>
            <h3 className="text-2xl font-semibold tracking-[-0.02em] transition-colors group-hover:text-apple-blue">
              {project.name}
            </h3>
